...
async function signIn() {
  if (!supabase) {
    setAuthStatus("Supabase n'est pas prêt. Vérifie supabase-config.js et recharge la page.");
    return;
  }
  ...
  try {
    ...
    setAuthStatus(`Connexion impossible : ${error.message || "erreur réseau"}`);
  }
}
...
async function signUp() {
  ...
  try {
    ...
    setAuthStatus(`Création impossible : ${error.message || "erreur réseau"}`);
  }
}
...
async function signOut() {
  ...
  try {
    ...
    setAuthStatus(`Déconnexion impossible : ${error.message || "erreur réseau"}`);
  }
}
